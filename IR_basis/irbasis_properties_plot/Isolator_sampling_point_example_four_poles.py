...
## "numerical exact"
Giwn = 1/4*(1/(iwn+1)+1/(iwn-1) + 1/(iwn+w_max)+1/(iwn-w_max))
Giwn_exact_on_IR = 1/4*(1/(iwn_IR+1)+1/(iwn_IR-1) + 1/(iwn_IR+w_max)+1/(iwn_IR-w_max))
#Giwn = 2*(iwn-sign(iwn*sqrt(iwn**2-1))

Gtau = -1/4*( exp(-tau)/(1+exp(-beta)) + exp(tau)/(1+exp(beta)) +\
             exp(-w_max*tau)/(1+exp(-w_max*beta)) + 1/(exp(-w_max*tau)+exp(w_max*(beta-tau))) )
Gtau_exact_on_IR = +1/4*( exp(-tau_IR)/(1+exp(-beta)) + exp(tau_IR)/(1+exp(beta)) +\
            exp(-w_max*tau_IR)/(1+exp(-w_max*beta)) + 1/(exp(-w_max*tau_IR)+exp(w_max*(beta-tau_IR))) )

## IR basis
rho_l = sqrt(1/w_max)*1/4*array([b.fermi_basis.vly(l, -1/w_max) + b.fermi_basis.vly(l, 1/w_max) +\
                                 b.fermi_basis.vly(l, -w_max/w_max) + b.fermi_basis.vly(l, w_max/w_max)for l in range(lmax+1)])
...
f_ax2.set_xlim([-4*10**3,4*10**3])
f_ax2.set_ylim([-0.75,0.75])
#f_ax2.set_ylim([-3,3])
